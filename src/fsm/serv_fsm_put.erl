...
%% API
-export([start_link/5, start_link/6, put/3, put/4]).
...
put(Client, StatName, Op) ->
    put(Client, StatName, Op, undefined).

put(Client, StatName, Op, Val) ->
    ReqID = mk_reqid(),
    serv_fsm_put_sup:start_fsm_put([ReqID, self(), Client, StatName, Op, Val]),
    {ok, ReqID}.
...
prepare(timeout, SD0=#state{client=Client,
    ...
    Preflist = riak_core_apl:get_apl(DocIdx, ?N, serv_vnode_stat),
...
execute(timeout, SD0=#state{req_id=ReqID,
    ...
    case Val of
	undefined ->
	    serv_vnode_stat:Op(Preflist, ReqID, StatName);
	_ ->
	    serv_vnode_stat:Op(Preflist, ReqID, StatName, Val)
    end,
