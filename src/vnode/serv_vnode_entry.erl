...
combined_lf({Client, _Entry, _Regexp}, [_Entry, _Host, _, _User, _Time, Req, Code, BodySize, _Referer, Agent]) ->
    serv:sadd(Client, "agents", Agent),
    serv:incrby(Client, "total_sent", list_to_integer(BodySize)),
    [Method, _Resource, _Protocol] = string:tokens(Req, " "),
    serv:incr(Client, Method),
    case Code of
	[$2, _, _] ->
	    serv:incr(Client, "200");
	[$3, _, _] ->
	    serv:incr(Client, "300");
	[$4, _, _] ->
	    serv:incr(Client, "400");
	[$5, _, _] ->
	    serv:incr(Client, "500")
    end,
    serv:incr(Client, "total_reqs").
