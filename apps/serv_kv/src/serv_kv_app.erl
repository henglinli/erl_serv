...
start(_Type, _StartArgs) ->
    ...
    FSM_Limit = app_helper:get_env(serv_kv, fsm_limit, ?DEFAULT_FSM_LIMIT),
    case FSM_Limit of
        undefined ->
            ok;
        _ ->
    ...
    case app_helper:get_env(serv_kv, direct_stats, false) of
        true ->
            ok;
        false ->
    ...
    case app_helper:get_env(serv_kv, add_paths) of
    ...
    StorageBackend = app_helper:get_env(serv_kv, storage_backend, leveldb),
    ...
    case riak_kv_sup:start_link() of
        {ok, Pid} ->
            ...
            riak_core_capability:register({serv_kv, vnode_vclocks},
                                          [true, false],
                                          false,
                                          {serv_kv,
                                           vnode_vclocks,
            ...
            riak_core_capability:register({serv_kv, legacy_keylisting},
                                          [false],
                                          false,
                                          {serv_kv,
                                           legacy_keylisting,
            ...
            riak_core_capability:register({serv_kv, listkeys_backpressure},
                                          [true, false],
                                          false,
                                          {serv_kv,
            ...
            riak_core_capability:register({serv_kv, index_backpressure},
                                          [true, false],
                                          false),
            ...
            riak_core_capability:register({serv_kv, anti_entropy},
                                          [enabled_v1, disabled],
                                          disabled),

            riak_core_capability:register({serv_kv, handoff_data_encoding},
                                          [encode_raw, encode_zlib],
                                          encode_zlib),

            riak_core_capability:register({serv_kv, object_format},
                                          get_object_format_modes(),
                                          v0),

            riak_core_capability:register({serv_kv, secondary_index_version},
                                          [v3, v2, v1],
                                          v1),

            riak_core_capability:register({serv_kv, vclock_data_encoding},
                                          [encode_zlib, encode_raw],
                                          encode_zlib),

            riak_core_capability:register({serv_kv, crdt},
                                          [?TOP_LEVEL_TYPES, [pncounter], []],
                                          []),

            riak_core_capability:register({serv_kv, put_fsm_ack_execute},
                                          [enabled, disabled],
                                          disabled),

            HealthCheckOn = app_helper:get_env(serv_kv, enable_health_checks, false),
            %% Go ahead and mark the riak_kv service as up in the node watcher.
            %% The riak_core_ring_handler blocks until all vnodes have been started
            %% synchronously.
            riak_core:register(serv_kv, [
...
prep_stop(_State) ->
    try %% wrap with a try/catch - application carries on regardless,
        ...
        riak_core_node_watcher:service_down(serv_kv),
...
stop(_State) ->
    lager:info("Stopped  application serv_kv.\n", []),
    ok.
...
check_kv_health(_Pid) ->
    ...
    {Low, High} = app_helper:get_env(serv_kv, vnode_mailbox_limit, {1, 5000}),
    case lists:member(serv_kv, riak_core_node_watcher:services(node())) of
        true ->
    ...
    case {Passed, Mode} of
        {false, enabled} ->
            lager:info("Disabling serv_kv due to large message queues. "
                       "Offending vnodes: ~p", [SlowVNs]);
        {true, disabled} ->
            lager:info("Re-enabling serv_kv after successful health check");
        _ ->
            ok
    end,
    Passed.
...
get_object_format_modes() ->
    ...
    case app_helper:get_env(serv_kv, object_format, v1) of
        v0 -> [v0,v1];
        v1 -> [v1,v0]
    end.
